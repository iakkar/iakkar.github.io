...
function randomCountry(countriesToShowList,countryObjectsList) {
    let num;
    do {
        num = Math.floor(Math.random()*(countryObjectsList.length -2) + 1);
        // console.log(num);
    } while (countriesToShowList.includes(countryObjectsList[num]["code"]));
    return num;
}

function findCountryName(neighbourCode,countryObjectsList) {
    
    for (let c of countryObjectsList){
        if (c.code === neighbourCode) {
            return c.name.indexOf('(') !== -1 ? c.name.slice(0, c.name.indexOf('(')) : c.name;
        }
    }
...
window.onload = function () {
    ...
    const countryNameField = document.getElementById("country-name-field");
    const countryFlagField = document.getElementById("country-flag-field");
    const scoreInfo = document.getElementById("score-info");
    const roundInfo = document.getElementById("round-info");
    const neighboursPanel = document.getElementById("neighbours-panel");
    ...
    let shuffledCountries = [...shuffleArray(countryObjects)];
    ...
    if (!localStorage.highScore) {
        // console.log("Initiated high score.")
        localStorage.highScore = 0;
        highScoreInfo.innerText = localStorage.highScore;
    } else {
        highScoreInfo.innerText = localStorage.highScore;
    }
    ...
    function startGame() {
        ...
        let country = shuffledCountries.shift();
        // console.log(shuffledCountries);

        let guesses = 0;
        let corrGuesses = 0;

        nextCountryBtn.setAttribute("disabled","");
        neighboursPanel.style.pointerEvents = "auto";

        fetch(url + country.code)
        .then((reply) => {
            if (reply.status === 200){
                return reply.json();
        ...
        .then((reply) => {
            ...
            } else if (typeof reply[0]['borders'] === 'undefined'){
                startGame();
            } else {
                countryNameField.innerText = country.name.indexOf('(') !== -1 ? country.name.slice(0, country.name.indexOf('(')) : country.name;
                countryFlagField.innerText = country2emoji2(country.code);

                // console.log(reply[0]['borders']);
                let neighbouringCountries = [...reply[0]['borders']];
                let countriesToShow = new Array();

                for (let i = 0; i < neighbouringCountries.length; i++){

                    let progressBox = document.createElement("div");
                    progressBox.style.width = `${100 / neighbouringCountries.length}%`
                    progressBar.appendChild(progressBox);

                    if (neighbouringCountries[i] === "UNK"){
                        neighbouringCountries[i] = "KOS";
                    }


                    for (let ctr of countryObjects){
                        if (ctr["code3"] === neighbouringCountries[i]){
                            neighbouringCountries[i] = ctr["code"];
                        }
                    }
                    countriesToShow.push(neighbouringCountries[i]);
                }
    
                for (let i = 0; i < neighbouringCountries.length; i++){
                    countriesToShow.push((countryObjects[randomCountry(countriesToShow,countryObjects)])["code"]);
                }
                
                // console.log(neighbouringCountries);
                // console.log(countriesToShow);

                countriesToShow = shuffleArray(countriesToShow);
                // console.log(countriesToShow);
    
                for (let neighbour of countriesToShow) {
                    // console.log(neighbour);
    
                    let countryBox = document.createElement("div");
                    countryBox.setAttribute("class","neighbour");
                    countryBox.setAttribute("id",neighbour);
                    countryBox.innerHTML = `
                    <div class='neighbour-flag'>
                        ${country2emoji2(neighbour)}
                    </div>
                    <div>
                        ${findCountryName(neighbour,countryObjects)}
                    </div>`;
                    neighboursPanel.appendChild(countryBox);
                }
    
                let flags = document.getElementsByClassName("neighbour");
                for(let flag of flags) {
                    flag.addEventListener ('click', function () {
                        //console.log(this.id);
                        
                        if (neighbouringCountries.includes(this.id)) {
                            progressBar.children[corrGuesses].style.backgroundColor = "#4caf50";
                            corrGuesses++;
                            score++;
                        ...
                        if (++guesses === neighbouringCountries.length) {
                            if (corrGuesses === neighbouringCountries.length){
                                messageField.innerText = "Βρήκες όλες τις γειτονικές χώρες. Συγχαρητήρια!";
                            } else if (corrGuesses >= neighbouringCountries.length/2) {
                                messageField.innerText = "Βρήκες αρκετές από τις γειτονικές χώρες. Μπράβο!";
                            } else if (corrGuesses >= 1) {
                                messageField.innerText = "Βρήκες μερικές από τις γειτονικές χώρες. Καλή προσπάθεια!";
                            } else {
                                messageField.innerText = "Δεν βρήκες κάποια γειτονική χώρα. Ξαναπροσπάθησε!";
                            }

                            for (let ctr of flags) {
                                if (neighbouringCountries.includes(ctr.id)) {
                                    ctr.style.backgroundColor = "lime";
                                } else {
                            ...
                            if(round >= 10){
                                messageField.innerText = "Τέλος παιχνιδιού!";
                                if (score > localStorage.highScore) {
                                    localStorage.highScore = score;
                                }
                            } else {
                                nextCountryBtn.removeAttribute("disabled");
                            }
                        }
